...
import { getReinf, putReinf } from '@syntesis/s-reinf'

class DatagridReinfLogsStore extends DatagridClientSideStore {
  constructor() {
    super({
  ...
  async updateItem(values, cb = {}) {
    try {
      const { id } = this.item
      const { response } = await getReinf({ id })
      await putReinf({ ...response, ...values })
...
const store = new DatagridReinfLogsStore()
