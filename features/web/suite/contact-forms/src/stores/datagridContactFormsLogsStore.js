...
import { getCrmForms, putCrmForms } from '@syntesis/s-crm-forms'

class DatagridContactFormsLogsStore extends DatagridClientSideStore {
  constructor() {
    super({
      endpoint: 'Crm/CrmForms/Logs',
      columns: [
        {
          Header: 'Data/Hora',
          accessor: 'dateLog',
  ...
  async updateItem(values, cb = {}) {
    try {
      const { id } = this.item
      const { response } = await getCrmForms({ id })
      await putCrmForms({ ...response, ...values })
...
const store = new DatagridContactFormsLogsStore()
