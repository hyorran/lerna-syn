...
import { getAssignmentOrigins, putAssignmentOrigins } from '@syntesis/s-assignment-origins'

class DatagridAssignmentOriginsLogsStore extends DatagridClientSideStore {
  constructor() {
    super({
      endpoint: 'Projects/AssignmentOrigins/Logs',
      columns: [
        {
          Header: 'Data/Hora',
          accessor: 'dateLog',
  ...
  async updateItem(values, cb = {}) {
    try {
      const { id } = this.item
      const { response } = await getAssignmentOrigins({ id })
      await putAssignmentOrigins({ ...response, ...values })
...
const store = new DatagridAssignmentOriginsLogsStore()
