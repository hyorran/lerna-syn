...
import distribuitionsFilterStore from '../../stores/formDistribuitionsFilterStore'

import styles from './styles'

@inject('distribuitionsFilterStore')
@observer
class FormFilterContainer extends Component {
  ...
  onFormChange(controlName, control) {
    this.props.distribuitionsFilterStore.changeFormControl(controlName, control)
  }

  render() {
    const {
      distribuitionsFilterStore: store,
    } = this.props
...
FormFilterContainer.propTypes = {
  ...
  distribuitionsFilterStore: MobxPropTypes.objectOrObservableObject
}
...
export default flow(
  withStyles(styles),
  withStores({ distribuitionsFilterStore })
)(FormFilterContainer)
