...
import { getDistribuitions, putDistribuitions } from '@syntesis/s-distribuitions'

class DatagridDistribuitionsLogsStore extends DatagridClientSideStore {
  constructor() {
    super({
      endpoint: 'Lawsuit/Distribuitions/Logs',
      // params: {
      //   PageSize: 15
      // },
      columns: [
  ...
  async updateItem(values, cb = {}) {
    try {
      const { id } = this.item
      const { response } = await getDistribuitions({ id })
      await putDistribuitions({ ...response, ...values })
...
const store = new DatagridDistribuitionsLogsStore()
