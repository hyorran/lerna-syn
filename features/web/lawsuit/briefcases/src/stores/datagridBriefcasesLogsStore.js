...
import { getBriefcases, putBriefcases } from '@syntesis/s-briefcases'

class DatagridBriefcasesLogsStore extends DatagridClientSideStore {
  constructor() {
    super({
      endpoint: 'Lawsuit/Briefcases/Logs',
      columns: [
        {
          Header: 'Data/Hora',
          accessor: 'dateLog',
  ...
  async updateItem(values, cb = {}) {
    try {
      const { id } = this.item
      const { response } = await getBriefcases({ id })
      await putBriefcases({ ...response, ...values })
...
const store = new DatagridBriefcasesLogsStore()
