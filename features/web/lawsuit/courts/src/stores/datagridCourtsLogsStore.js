...
import { getCourts, putCourts } from '@syntesis/s-courts'

class DatagridCourtsLogsStore extends DatagridClientSideStore {
  constructor() {
    super({
      endpoint: 'Lawsuit/Courts/Logs',
      // params: {
      //   PageSize: 15
      // },
      columns: [
  ...
  async updateItem(values, cb = {}) {
    try {
      const { id } = this.item
      const { response } = await getCourts({ id })
      await putCourts({ ...response, ...values })
...
const store = new DatagridCourtsLogsStore()
