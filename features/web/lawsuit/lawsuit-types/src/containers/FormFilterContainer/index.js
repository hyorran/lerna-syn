...
import lawsuitTypesFilterStore from '../../stores/formLawsuitTypesFilterStore'

import styles from './styles'

@inject('lawsuitTypesFilterStore')
@observer
class FormFilterContainer extends Component {
  ...
  onFormChange(controlName, control) {
    this.props.lawsuitTypesFilterStore.changeFormControl(controlName, control)
  }

  render() {
    const {
      lawsuitTypesFilterStore: store,
    } = this.props
...
FormFilterContainer.propTypes = {
  ...
  lawsuitTypesFilterStore: MobxPropTypes.objectOrObservableObject
}
...
export default flow(
  withStyles(styles),
  withStores({ lawsuitTypesFilterStore })
)(FormFilterContainer)
