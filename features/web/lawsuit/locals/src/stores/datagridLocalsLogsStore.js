...
import { getLocals, putLocals } from '@syntesis/s-locals'

class DatagridLocalsLogsStore extends DatagridClientSideStore {
  constructor() {
    super({
      endpoint: 'Lawsuit/Locals/Logs',
      // params: {
      //   PageSize: 15
      // },
      columns: [
  ...
  async updateItem(values, cb = {}) {
    try {
      const { id } = this.item
      const { response } = await getLocals({ id })
      await putLocals({ ...response, ...values })
...
const store = new DatagridLocalsLogsStore()
