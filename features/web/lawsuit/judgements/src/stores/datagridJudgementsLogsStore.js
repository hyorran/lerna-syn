...
import { getJudgements, putJudgements } from '@syntesis/s-judgements'

class DatagridJudgementsLogsStore extends DatagridClientSideStore {
  constructor() {
    super({
      endpoint: 'Lawsuit/Judgements/Logs',
      // params: {
      //   PageSize: 15
      // },
      columns: [
  ...
  async updateItem(values, cb = {}) {
    try {
      const { id } = this.item
      const { response } = await getJudgements({ id })
      await putJudgements({ ...response, ...values })
...
const store = new DatagridJudgementsLogsStore()
