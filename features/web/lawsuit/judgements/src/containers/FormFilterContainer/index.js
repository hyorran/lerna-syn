...
import judgementsFilterStore from '../../stores/formJudgementsFilterStore'

import styles from './styles'

@inject('judgementsFilterStore')
@observer
class FormFilterContainer extends Component {
  ...
  onFormChange(controlName, control) {
    this.props.judgementsFilterStore.changeFormControl(controlName, control)
  }

  render() {
    const {
      judgementsFilterStore: store,
    } = this.props
...
FormFilterContainer.propTypes = {
  ...
  judgementsFilterStore: MobxPropTypes.objectOrObservableObject
}
...
export default flow(
  withStyles(styles),
  withStores({ judgementsFilterStore })
)(FormFilterContainer)
