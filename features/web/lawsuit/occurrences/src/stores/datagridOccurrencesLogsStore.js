...
import { getOccurrences, putOccurrences } from '@syntesis/s-occurrences'

class DatagridOccurrencesLogsStore extends DatagridClientSideStore {
  constructor() {
    super({
      endpoint: 'Lawsuit/Occurrences/Logs',
      // params: {
      //   PageSize: 15
      // },
      columns: [
  ...
  async updateItem(values, cb = {}) {
    try {
      const { id } = this.item
      const { response } = await getOccurrences({ id })
      await putOccurrences({ ...response, ...values })
...
const store = new DatagridOccurrencesLogsStore()
