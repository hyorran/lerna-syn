...
import { getForums, putForums } from '@syntesis/s-forums'

class DatagridForumsLogsStore extends DatagridClientSideStore {
  constructor() {
    super({
      endpoint: 'Lawsuit/Forums/Logs',
      // params: {
      //   PageSize: 15
      // },
      columns: [
  ...
  async updateItem(values, cb = {}) {
    try {
      ...
      await putForums({ ...response, ...values })
...
const store = new DatagridForumsLogsStore()
