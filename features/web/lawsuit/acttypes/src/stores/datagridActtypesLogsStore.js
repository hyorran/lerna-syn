...
import { getActtypes, putActtypes } from '@syntesis/s-acttypes'

class DatagridActtypesLogsStore extends DatagridClientSideStore {
  constructor() {
    super({
      endpoint: 'Lawsuit/Acttypes/Logs',
      // params: {
      //   PageSize: 15
      // },
      columns: [
  ...
  async updateItem(values, cb = {}) {
    try {
      const { id } = this.item
      const { response } = await getActtypes({ id })
      await putActtypes({ ...response, ...values })
...
const store = new DatagridActtypesLogsStore()
