...
import acttypesFilterStore from '../../stores/formActtypesFilterStore'

import styles from './styles'

@inject('acttypesFilterStore')
@observer
class FormFilterContainer extends Component {
  ...
  onFormChange(controlName, control) {
    this.props.acttypesFilterStore.changeFormControl(controlName, control)
  }

  render() {
    const {
      acttypesFilterStore: store,
    } = this.props
...
FormFilterContainer.propTypes = {
  ...
  acttypesFilterStore: MobxPropTypes.objectOrObservableObject
}
...
export default flow(
  withStyles(styles),
  withStores({ acttypesFilterStore })
)(FormFilterContainer)
