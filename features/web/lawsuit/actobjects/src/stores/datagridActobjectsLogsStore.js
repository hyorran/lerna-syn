...
import { getActobjects, putActobjects } from '@syntesis/s-actobjects'

class DatagridActobjectsLogsStore extends DatagridClientSideStore {
  constructor() {
    super({
      endpoint: 'Lawsuit/Actobjects/Logs',
      // params: {
      //   PageSize: 15
      // },
      columns: [
  ...
  async updateItem(values, cb = {}) {
    try {
      const { id } = this.item
      const { response } = await getActobjects({ id })
      await putActobjects({ ...response, ...values })
...
const store = new DatagridActobjectsLogsStore()
