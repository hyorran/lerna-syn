...
import decisionsFilterStore from '../../stores/formDecisionsFilterStore'

import styles from './styles'

@inject('decisionsFilterStore')
@observer
class FormFilterContainer extends Component {
  ...
  onFormChange(controlName, control) {
    this.props.decisionsFilterStore.changeFormControl(controlName, control)
  }

  render() {
    const {
      decisionsFilterStore: store,
    } = this.props
...
FormFilterContainer.propTypes = {
  ...
  decisionsFilterStore: MobxPropTypes.objectOrObservableObject
}
...
export default flow(
  withStyles(styles),
  withStores({ decisionsFilterStore })
)(FormFilterContainer)
