...
import { getDecisions, putDecisions } from '@syntesis/s-decisions'

class DatagridDecisionsLogsStore extends DatagridClientSideStore {
  constructor() {
    super({
      endpoint: 'Lawsuit/Decisions/Logs',
      // params: {
      //   PageSize: 15
      // },
      columns: [
  ...
  async updateItem(values, cb = {}) {
    try {
      const { id } = this.item
      const { response } = await getDecisions({ id })
      await putDecisions({ ...response, ...values })
...
const store = new DatagridDecisionsLogsStore()
