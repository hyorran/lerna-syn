...
import { getLawsuitClientConditions, putLawsuitClientConditions } from '@syntesis/s-lawsuit-client-conditions'

class DatagridLawsuitClientConditionsLogsStore extends DatagridClientSideStore {
  constructor() {
    super({
      endpoint: 'Lawsuit/LawsuitClientConditions/Logs',
      // params: {
      //   PageSize: 15
      // },
      columns: [
                ...
                <Button
                  ...
                  onClick={
                    () => openHistoryItemModal({
                      item: original,
                      operation: value,
                      name: () => `${ this.getItem.code } - ${ this.getItem.clientCondition } - ${ this.getItem.defendantCondition }`,
  ...
  async updateItem(values, cb = {}) {
    try {
      const { id } = this.item
      const { response } = await getLawsuitClientConditions({ id })
      await putLawsuitClientConditions({ ...response, ...values })
...
const store = new DatagridLawsuitClientConditionsLogsStore()
