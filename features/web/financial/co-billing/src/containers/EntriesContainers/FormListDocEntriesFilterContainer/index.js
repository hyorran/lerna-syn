...
@observer
class FormListDocEntriesFilterContainer extends Component {
  constructor(props) {
    super(props)
  ...
  onFormChange(controlName, control) {
    this.props.formListDocEntriesFilterStore.changeFormControl(controlName, control)
  }

  render() {
    const {
      formListDocEntriesFilterStore: store,
    } = this.props
...
FormListDocEntriesFilterContainer.propTypes = {
...
FormListDocEntriesFilterContainer.defaultProps = {
  dialogId: undefined
}

export default flow(
  withStyles(styles),
  withStores({ formListDocEntriesFilterStore })
)(FormListDocEntriesFilterContainer)
