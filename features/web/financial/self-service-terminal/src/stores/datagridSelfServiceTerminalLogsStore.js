...
import { getSelfServiceTerminal, putSelfServiceTerminal } from '@syntesis/s-self-service-terminal'

class DatagridSelfServiceTerminalLogsStore extends DatagridClientSideStore {
  constructor() {
    super({
      endpoint: 'Financial/FinancialSalePoints/Logs',
      // params: {
      //   PageSize: 15
      // },
      columns: [
  ...
  async updateItem(values, cb = {}) {
    try {
      const { id } = this.item
      const { response } = await getSelfServiceTerminal({ id })
      ...
      await putSelfServiceTerminal({ ...response, ...values })
...
const store = new DatagridSelfServiceTerminalLogsStore()
